...
class MapGenerator2(QThread):
	...
	def configureMap(self, seed= -1, faultNum= -1, percentWater= 60, size= (1000,500)):
		# Seed Generation or Interpretation
		if type(seed) == str:
			self.seedname = seed
		else:
		...
		if self.faultNum < 0:
			self.faultNum = random.randint(10,70) * 100
	...
	def AddFaults(self, faultNum):
		...
		for i in range(faultNum):
			if (i in milestones):
				self.printUpdate.emit("Fault " + str(i - 1) + " (" + str(i * 100 // faultNum) + "%)")
			self.GenerateFault(i)
